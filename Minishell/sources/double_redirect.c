...
char **split_double_redirect(char *str)
{
    int i = 1;
...
void do_the_double_redirect(main_t *shell, int i)
{
    ...
    char **redirect_a = split_double_redirect(str);
    int fd = 0;
    int save_std = 0;

    shell->split = new_put_in_array(redirect_a[0]);
    fd = open(redirect_a[1], O_CREAT|O_APPEND|O_RDWR, 0755);
    save_std = dup(1);
    dup2(fd, 1);
    check_builtin(shell);
...
int check_further_double_redirection(main_t *shell, int i, int *j)
{
    while (shell->split1[i][*j] != '\0') {
        if (shell->split1[i][*j] == '>' && shell->split1[i][*j + 1] == '>') {
            return (0);
        }
        (*j)++;
    }
...
int check_double_redirection(main_t *shell)
{
    ...
    while (shell->split1[i] != NULL) {
        if ((check_further_double_redirection(shell, i, &j)) == 0) {
            shell->special = 3;
            return (0);
        }
        i++;
